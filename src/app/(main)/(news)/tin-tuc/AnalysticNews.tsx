...
export default function AnalysticNews({ listNews }: Props) {
  return (
    <>
      <div className="border-b-2 mb-2 mt-9">
        <div className="flex justify-between pb-2">
          ...
          <Link
            href="/tin-tuc/tim-kiem?type=phan-tich&page=0"
      ...
      <div className="pt-2">
        <div className="flex items-stretch overflow-x-auto flex-nowrap mb-3">
          {listNews && listNews.data && listNews.data.length > 0
            ? listNews.data.map((item) => <NewsItem itemNews={item} key={item.id} />)
            : ''}
        </div>
      </div>
    </>
